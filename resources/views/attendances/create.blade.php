                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        @foreach($employees as $employee)
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                @if($campaign->attendance_method === \App\Models\Campaign::ATTENDANCE_METHOD_CALL_TIME)
                                                    <td class="px-4 py-3">
                                                        <x-text-input type="number" step="0.01" min="0" name="targets[{{ $employee->id }}][call_time]" value="{{ old('targets.'.$employee->id.'.call_time') }}" class="w-full" placeholder="7.5" />
                                                    </td>
                                                    <td class="px-4 py-3">
                                                        <x-text-input type="number" step="0.01" min="0" name="targets[{{ $employee->id }}][daily_salary]" value="{{ old('targets.'.$employee->id.'.daily_salary') }}" class="w-full" placeholder="500" />
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
